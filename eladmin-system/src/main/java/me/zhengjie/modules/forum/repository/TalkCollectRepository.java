...
public interface TalkCollectRepository extends JpaRepository<TalkCollect, Long>, JpaSpecificationExecutor<TalkCollect> {
    ...
    @Query(value = "select t.id as id ,t.user_id as userId,t.title,t.subject,t.content,t.is_anonymous as isAnonymous,t.create_time as createTime,u.username,u.nick_name as nickname,u.avatar_path as avatarPath," +
            ...
            "where t.id in(select talk_id from talk_collect where user_id=:userId) " +
            "and (if(:searchStr !='',t.title like concat('%',:searchStr,'%') or t.subject like concat('%',:searchStr,'%') ,1=1)) ",
            countQuery = "select count(id) from talk t where id in(select talk_id from talk_collect where user_id=:userId) " +
                    "and (if(:searchStr !='',t.title like concat('%',:searchStr,'%') or t.subject like concat('%',:searchStr,'%'),1=1))", nativeQuery = true
    )
    Page<Map<String, Object>> myCollectPage(@Param("searchStr") String searchStr, @Param("userId")Long userId, Pageable pageable);

    @Modifying
    @Query(value = "delete from talk_collect where user_id=:userId and talk_id=:talkId"
            , nativeQuery = true
    )
    void cancel(@Param("userId")Long userId, @Param("talkId")Long talkId);
}